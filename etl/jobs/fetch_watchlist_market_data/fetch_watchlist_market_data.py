...
def get_assets_needing_update(assets):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM watchlist_market_data w
            WHERE w.symbol = ANY(%s) AND (w.updated_at IS NULL OR w.updated_at < %s)
...
def insert_or_update_data(cursor, connection, processed_data):
    ...
    try:
        cursor.execute("""
            INSERT INTO watchlist_market_data (symbol, asset_type, price, percent_change, change, high, low, updated_at)
...
def run(message_payload):
    ...
    fetch_and_insert_data(
        ...
        get_realtime_stock_data,
        get_realtime_crypto_data,
        None  # Forex data is not required for this job
    )
